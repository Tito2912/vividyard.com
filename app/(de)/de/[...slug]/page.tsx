...
export async function generateStaticParams() {
  ...
  return all.filter((x) => x.slug[0] === 'de' && x.slug.length > 1).map((x) => ({ slug: x.slug.slice(1) }));
}
...
export default async function GermanCatchAllPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
