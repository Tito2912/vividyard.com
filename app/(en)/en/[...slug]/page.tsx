...
export async function generateStaticParams() {
  ...
  return all.filter((x) => x.slug[0] === 'en' && x.slug.length > 1).map((x) => ({ slug: x.slug.slice(1) }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  const { slug } = await params;
  const meta = await getDocMetaByRouteSegments(['en', ...slug]);
  if (!meta) return {};
...
export default async function EnglishCatchAllPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
  const post = await getPostByRouteSegments(['en', ...slug]);
