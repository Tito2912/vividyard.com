...
export async function generateStaticParams() {
  ...
  return all.filter((x) => x.slug[0] === 'es' && x.slug.length > 1).map((x) => ({ slug: x.slug.slice(1) }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  const { slug } = await params;
  const meta = await getDocMetaByRouteSegments(['es', ...slug]);
  if (!meta) return {};
...
export default async function SpanishCatchAllPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
  const post = await getPostByRouteSegments(['es', ...slug]);
